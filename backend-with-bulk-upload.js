...
const GOOGLE_REDIRECT_URI = process.env.GOOGLE_REDIRECT_URI || 'https://mustabackend-nenb.onrender.com/auth/google/callback';
...
async function handleAPI(req, res, pathname, method, parsed, ip, origin) {
    ...
    if (method === 'GET' && pathname === '/auth/google/callback') {
        try {
            ...
            if (!code) {
                res.writeHead(302, { 'Location': FRONTEND_URL + '?error=google_auth_failed' });
                return res.end();
            }

            // Exchange code for tokens
            const tokenData = await httpsRequest('POST', 'https://oauth2.googleapis.com/token', {
                code,
                client_id: GOOGLE_CLIENT_ID,
            ...
            const userInfo = await httpsRequest('GET', `https://www.googleapis.com/oauth2/v2/userinfo?access_token=${tokenData.access_token}`);

            // Check if user exists
            let r = await query('SELECT * FROM users WHERE email=$1', [userInfo.email]);
            let u;
            ...
            if (r.rows.length > 0) {
                ...
                // New user - register
                const username = userInfo.email.split('@')[0] + '_' + Math.random().toString(36).substr(2, 4);
                r = await query(
                    'INSERT INTO users (username, email, password, profile_photo, created_at) VALUES ($1,$2,$3,$4,NOW()) RETURNING *',
                    [username, userInfo.email, hashPassword(crypto.randomBytes(32).toString('hex')), userInfo.picture]
                );
                u = r.rows[0];
            }
            ...
            const frontendUrl = FRONTEND_URL === '*' ? 'https://djmusta.com' : FRONTEND_URL;
            res.writeHead(302, { 'Location': `${frontendUrl}?token=${token}` });
            return res.end();

        } catch(e) {
            console.error('Google OAuth error:', e);
            const frontendUrl = FRONTEND_URL === '*' ? 'https://djmusta.com' : FRONTEND_URL;
            res.writeHead(302, { 'Location': `${frontendUrl}?error=google_auth_failed` });
            return res.end();
        }
    }
